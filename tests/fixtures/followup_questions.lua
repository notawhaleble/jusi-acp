...
local ok, failure = xpcall(function()
  ...
  result.diff = followup({"diff"})
  wait_for(function() return screen():find("diff", 1, true) end, "diff turn did not reach turns sheet")
  local source_tab = vim.api.nvim_get_current_tabpage()
  vim.fn.chansend(surface.job_id, "G")
  vim.wait(100)
  vim.fn.chansend(surface.job_id, "d")
  local diff_opened = vim.wait(15000, function() return vim.b.jusi_diff_side == "after" end, 20)
  result.diff_screen = screen()
  assert(diff_opened, "turn diff did not open in Vim\n" .. result.diff_screen)
  local diff_tab = vim.api.nvim_get_current_tabpage()
  local diff_wins = vim.api.nvim_tabpage_list_wins(diff_tab)
  assert(diff_tab ~= source_tab and #diff_wins == 2, "turn diff did not create two diff windows")
  for _, win in ipairs(diff_wins) do
    local diff_buf = vim.api.nvim_win_get_buf(win)
    assert(vim.wo[win].diff and not vim.bo[diff_buf].modifiable)
    local expected = vim.b[diff_buf].jusi_diff_side == "before" and "before" or "after α"
    assert(table.concat(vim.api.nvim_buf_get_lines(diff_buf, 0, -1, false), "\n") == expected)
  end
